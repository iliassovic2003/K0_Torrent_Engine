...
namespace k0 {
                                    ...
                                    const std::string& path)
    {
        ...
        if (fd < 0) { freeaddrinfo(res); throw NetworkError("socket() failed"); }
    ...
    std::vector<PeerAddress> HttpTracker::parse_dict_peers(const std::string& raw) {
        // raw is a bencoded list of dicts: [{"ip":"...", "port":<int>}, ...]
        std::vector<PeerAddress> peers;
        try {
            auto v = bencode::decode(raw);
            for (const auto& entry : v.as_list()) {
    ...
    TrackerResponse HttpTracker::parse_response(const std::string& body) {
        ...
        if (d.count("peers")) {
            ...
            if (peers_val.is_string())
                ...
                resp.peers = parse_dict_peers(bencode::encode(peers_val));
        }
                            ...
                            int32_t& completed)
    {
        ...
        size_t pos = scrape_url.rfind("/announce");
        ...
        try {
            auto            root = bencode::decode(body);
            const auto&     files = root.get("files").as_dict();
